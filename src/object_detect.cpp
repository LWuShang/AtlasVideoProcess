...
Result ObjectDetect::Preprocess(cv::Mat& frame)
{
    ...
    cv::Mat resizeMat;
    cv::resize(frame, resizeMat, cv::Size(g_modelWidth_, g_modelHeight_));
    if (resizeMat.empty()) {
    ...
    aclError ret = aclrtMemcpy(g_imageDataBuf_, g_imageDataSize_,
                               resizeMat.ptr<uint8_t>(), g_imageDataSize_, policy);
    if (ret != ACL_SUCCESS) {
                                 ...
                                 aclmdlDataset* modelOutput)
{
    ...
    for (uint32_t i = 0; i < totalBox; i++) {
        ...
        float scoreLine = 0.80;
        if (score < scoreLine) continue;
        // get the frame coordinates and converts them to the coordinates on the original frame
        ltPt.x = detectData[totalBox * TOPLEFTX + i] * widthScale;
        ltPt.y = detectData[totalBox * TOPLEFTY + i] * heightScale;
        rbPt.x = detectData[totalBox * BOTTOMRIGHTX + i] * widthScale;
        rbPt.y = detectData[totalBox * BOTTOMRIGHTY + i] * heightScale;
        ...
        cv::rectangle(frame, cv::Point(ltPt.x, ltPt.y), cv::Point(rbPt.x, rbPt.y), color, lineSolid);
        string className = yolov3Label[objIndex];
        string markString = Utils::ConvertNum2Str(score) + ":" + className;
        cv::putText(frame, markString, cv::Point(ltPt.x, ltPt.y + labelOffset),
